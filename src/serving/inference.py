...
try:
    ...
    model_path_skops = os.path.join(MODEL_DIR, "model.skops")
    
    # Kịch bản 1: Nếu mô hình được lưu bằng cloudpickle (.pkl)
    if os.path.exists(model_path_pkl):
        with open(model_path_pkl, "rb") as f:
            model = pickle.load(f)
        print(f"✅ Model loaded successfully DIRECTLY via pickle from {model_path_pkl}")
        
    # Kịch bản 2: Nếu mô hình được lưu bằng skops (.skops)
    elif os.path.exists(model_path_skops):
        import skops.io as sio
        model = sio.load(
            model_path_skops, 
            trusted=["xgboost.core.Booster", "xgboost.sklearn.XGBClassifier"]
        )
        print(f"✅ Model loaded successfully DIRECTLY via skops from {model_path_skops}")
        
    # Kịch bản 3: Không tìm thấy file nào
    else:
        raise FileNotFoundError(f"Could not find model.pkl or model.skops in {MODEL_DIR}")
        
except Exception as e:
    print(f"❌ Failed to load model from {MODEL_DIR}: {e}")
    raise Exception(f"CRITICAL ERROR: Cannot load model. Details: {e}")
...
def predict(input_dict: dict) -> str:
    ...
    Example:
        ...
        "Likely to churn"
    """
    print(f"DEBUG LOG - Type of model: {type(model)}")
